...
exports.streamFileTo0 = function(filename, out, opts, callback) {
    ...
    try {
        var fsin = fs.createReadStream(filename);
    } catch(exc) {
        return callback('error creating read stream');
    }
    callback = util.safeCallback(callback);
    
    fsin.on('error', function(err) { callback(err); });
    fsin.pipe(out, opts);
    out.on('error', function(err) { callback(err); });
    out.on('close', function(err) { callback(err); });
}
